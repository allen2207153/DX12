...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Vertex* vertMap = nullptr;//Vertexに変更
	...
	D3D12_DESCRIPTOR_RANGE descTablRange = {};
	descTablRange.NumDescriptors = 1;//テクスチャひとつ
	descTablRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;//種別はテクスチャ
	descTablRange.BaseShaderRegister = 0;//0番スロットから
	descTablRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;


	D3D12_ROOT_PARAMETER rootparam = {};
	rootparam.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootparam.DescriptorTable.pDescriptorRanges = &descTablRange;//デスクリプタレンジのアドレス
	...
	//Write-back(Read note)
	texHeapProp.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_WRITE_BACK;

	//transfer to L0,つまりCPUから直接行う
